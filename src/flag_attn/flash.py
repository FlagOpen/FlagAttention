...
def _fwd_kernel(
    ...
    if IS_CAUSAL:
        hi = next_multiple_of(P_SEQ + (start_m + 1) * BLOCK_M - 1, BLOCK_N) - BLOCK_N
    else:
        hi = next_multiple_of(N_CTX + P_SEQ, BLOCK_N) - BLOCK_N
        

    # loop over k, v and update accumulators
    offs_n_init = hi + offs_n_base
    ...
    for start_n in range(hi, -1, -BLOCK_N):
        ...
        alpha = tl.where(m_i_new == float("-inf"), 
                         1.0,
                         tl.math.exp2((m_i - m_i_new) * qk_scale)) # last block may have nan
        p = tl.where((m_i_new == float("-inf"))[:, None], 
                     0.0, 
                     tl.math.exp2((s - m_i_new[:, None]) * qk_scale)) # last block may have nan
    ...
    l = m_i * sm_scale + tl.math.log(l_i) # log(normalizer)
...
def _bwd_kernel(
    ...
    # offset pointers for batch/head
    D_ptrs = D + (off_z * H + off_h) * N_CTX
    l_ptrs = L + (off_z * H + off_h) * N_CTX
    ...
    for start_m in range(lo, N_CTX, BLOCK_M):
        ...
        p = tl.math.exp2(qk * qk_scale - l[:, None] * log2e) # (BLOCK_M, BLOCK_N)
